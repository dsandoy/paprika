	...
	function getChecked() {
		try {
			...
			if (localStorage.getItem('is_dark') === null) {
				localStorage.setItem('is_dark', JSON.stringify(false));
				return;
			}
			const checked = JSON.parse(localStorage.getItem('is_dark') as string);
			if (checked !== '') return checked;
		} catch {
			return false;
		}
	}
	let checked = writable<boolean>(getChecked());

	function toggleTheme() {
		$checked = !$checked;
		localStorage.setItem('is_dark', JSON.stringify($checked));
	}
</script>
